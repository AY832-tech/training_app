...
async function init() {
  ...
  await seedProgram();
  await seedStretches();
  await seedMachineExercises();
  await seedMuscles();
}

// マシン・ケーブル種目の追加シード（既存DBにも「無ければ追加」。name は UNIQUE なので OR IGNORE）
async function seedMachineExercises() {
  const machines = [
    // [name, category, unit, muscles]
    ['チェストプレス（マシン）', '胸', 'kg', '大胸筋・三角筋前部・上腕三頭筋'],
    ['ペックフライ（マシン）', '胸', 'kg', '大胸筋'],
    ['スミスマシンベンチプレス', '胸', 'kg', '大胸筋・三角筋前部・上腕三頭筋'],
    ['ケーブルクロスオーバー', '胸', 'kg', '大胸筋'],
    ['シーテッドロー（マシン）', '背中', 'kg', '広背筋・僧帽筋・上腕二頭筋'],
    ['アシストチンニング（マシン）', '背中', 'kg', '広背筋・上腕二頭筋'],
    ['ストレートアームプルダウン', '背中', 'kg', '広背筋'],
    ['マシンショルダープレス', '肩', 'kg', '三角筋前部・中部・上腕三頭筋'],
    ['リアデルトフライ（マシン）', '肩', 'kg', '三角筋後部・僧帽筋'],
    ['ケーブルサイドレイズ', '肩', 'kg', '三角筋中部'],
    ['レッグエクステンション', '脚', 'kg', '大腿四頭筋'],
    ['ハックスクワット', '脚', 'kg', '大腿四頭筋・殿筋'],
    ['スミスマシンスクワット', '脚', 'kg', '大腿四頭筋・殿筋・ハムストリングス'],
    ['ヒップアダクション（マシン）', '脚', 'kg', '内転筋'],
    ['ヒップアブダクション（マシン）', '脚', 'kg', '中殿筋'],
    ['ケーブルカール', '腕', 'kg', '上腕二頭筋'],
    ['プリーチャーカール（マシン）', '腕', 'kg', '上腕二頭筋'],
    ['トライセプスエクステンション（マシン）', '腕', 'kg', '上腕三頭筋'],
    ['アブドミナルクランチ（マシン）', '腹', 'kg', '腹直筋'],
    ['ロータリートルソー', '腹', 'kg', '腹斜筋'],
  ];
  await batch(machines.map(([name, cat, unit, m]) => ({
    sql: 'INSERT OR IGNORE INTO exercises (name, category, unit, muscles) VALUES (?, ?, ?, ?)',
    args: [name, cat, unit, m],
  })));
}
